...
config="./run/configs/CHESS_IR_CG_UT_deepseek.yaml"

num_workers=1 # Number of workers to use for parallel processing, set to 1 for no parallel processing

/mnt/cache/tonghao/envs/chess/bin/python3 -u ./src/main.py --data_mode ${data_mode} --data_path ${data_path} --config "$config" \
